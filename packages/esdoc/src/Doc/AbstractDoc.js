...
export default class AbstractDoc {
    ...
    for (const decorator of this._node.decorators) {
      ...
      switch (decorator.expression.type) {
        case 'Identifier':
          ...
          break;
        case 'CallExpression':
          value.name = babelGenerator(decorator.expression).code.replace(/[(][\S\s.]*/u, '');
          value.arguments = babelGenerator(decorator.expression).code.replace(/^[^(]+/u, '');
          break;
  ...
  _resolveLongname(name) {
    ...
    if (importPath.charAt(0) === '.' || importPath.charAt(0) === '/') {
      if (!upath.extname(importPath)) importPath += '.js';
