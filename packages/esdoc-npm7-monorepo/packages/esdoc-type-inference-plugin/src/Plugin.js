...
const ASTNodeContainer = require('esdoc-core/lib/Util/ASTNodeContainer.js').default;
const ASTUtil = require('esdoc-core/lib/Util/ASTUtil').default;
...
class Plugin {
  ...
  _inferenceMethod() {
    const docs = this._docs.filter((doc) => { return doc.kind === 'method'; });
  ...
  _inferenceGetter() {
    const docs = this._docs.filter((doc) => { return doc.kind === 'get'; });
  ...
  _inferenceMember() {
    const docs = this._docs.filter((doc) => { return doc.kind === 'member'; });
  ...
  _inferenceVariable() {
    const docs = this._docs.filter((doc) => { return doc.kind === 'variable'; });
      ...
      switch (param.type) {
        ...
        case 'AssignmentPattern':
          ...
          if (param.right.type.includes('Literal')) {
            ...
            result.defaultRaw = param.right.elements.map((elm) => { return elm.value; });
  ...
  _inferenceReturn(node) {
    ...
    ASTUtil.traverse(body, (node2, parent, path) => {
      ...
      if (node2.type !== 'ReturnStatement') return;

      if (!node2.argument) return;

      result.types = inferenceType(node2.argument).types;
    });
