(function() {
  ...
  input.addEventListener('keyup', (ev) => {
    ...
    for (let ii = 0; ii < len; ii = ii + 1) {
      const pair = searchIndex[ii];
    ...
    let innerHTML = '';
    for (const kind in html) {
      if (Object.prototype.hasOwnProperty.call( html, kind )) {
  ...
  input.addEventListener('keydown', (ev) => {
    ...
    if (ev.keyCode === 40) {
      // arrow down
      const selected5 = result.children[selectedIndex + 1];
      if (selected5 && selected5.classList.contains('search-separator')) {
        //const selected7 = result.children[selectedIndex + 2];
        selectedIndex++;
      }
      ...
      // arrow up
      const selected3 = result.children[selectedIndex - 1];
      if (selected3 && selected3.classList.contains('search-separator')) {
        //const selected6 = result.children[selectedIndex - 2];
        selectedIndex--;
      }
