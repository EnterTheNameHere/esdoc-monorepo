...
class Plugin {
  ...
  onPublish(ev) {
    ...
    const tmpResults = [];
    const docs = this._docs.filter((v) => { return ['method', 'function'].includes(v.kind); });
  ...
  _getParamsFromDoc(doc) {
    ...
    return params.map((v) => { return v.name; }).filter((v) => { return !v.includes('.'); }).filter((v) => { return !v.includes('['); });
  }
  ...
  _formatResult(tmpResults) {
    ...
    for (const result of tmpResults) {
      ...
      const fileDoc = this._docs.find((tag) => { return tag.kind === 'file' && tag.name === filePath; });
  ...
  _showResult() {
    ...
    for (const result of this._results) {
      console.warn(`[33mwarning: signature mismatch: ${result.name} ${result.filePath}#${result.lines[0].lineNumber}[32m`);
      for (const line of result.lines) {
        console.warn(`${line.lineNumber}| ${line.line}`);
      }
      console.warn('[0m');
    }
  }
