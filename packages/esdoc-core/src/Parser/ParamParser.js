...
export default class ParamParser {
  ...
  static parseParamValue(value, type = true, name = true, desc = true) {
    ...
    // e.g {number}
    if (type) {
      const reg = /^\{([^@]*?)\}(\s+|$)/u; // ``@`` is special char in ``{@link foo}``
  ...
  static parseParam(typeText = null, paramName = null, paramDesc = null) {
    ...
    if (typeText) {
      ...
      typeText = typeText.replace(/^[?!]/u, '');

      // check record and union
      if (typeText[0] === '{') {
        result.types = [typeText];
      } else if (typeText[0] === '(') {
        typeText = typeText.replace(/^[(]/u, '').replace(/[)]$/u, '');
    ...
    if (paramName) {
      ...
      if (paramName[0] === '[') {
        ...
        paramName = paramName.replace(/^[[]/u, '').replace(/[\]]$/u, '');
      } else {
