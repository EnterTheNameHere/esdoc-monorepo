...
class Plugin {
  ...
  _getRepositoryURL(packageObj) {
    ...
    const url = packageObj.repository.url || packageObj.repository;
    if (typeof url !== 'string') return null;

    if (url.indexOf('git@github.com:') === 0) { // url: git@github.com:foo/bar.git
      const matched = url.match(/^git@github\.com:(.*)\.git$/u);
      return `https://github.com/${matched[1]}`;
    } else if (url.match(/^[\w\d\-_]+\/[\w\d\-_]+$/u)) { // url: foo/bar
      return `https://github.com/${url}`;
    } else if (url.match(/^git\+https:\/\/github.com\/.*\.git$/u)) { // git+https://github.com/foo/bar.git
      const matched = url.match(/^git\+(https:\/\/github.com\/.*)\.git$/u);
      return matched[1];
    } else if (url.match(/(https?:\/\/.*$)/u)) { // other url
      const matched = url.match(/(https?:\/\/.*$)/u);
      return matched[1];
    }
  ...
  _addMetaTag($) {
    ...
    for (const metaProp of metaProps) {
      const prop = Object.keys(metaProp).map((key) => { return `${key}="${metaProp[key]}"`; }).join(' ');
